...
//! subaccounts to pay the transaction fee for the already enabled features (e.g. cyrux mining).
...
pub mod pallet {
	...
	mod test {
		...
		// Pallets
		use crate::mock::cyruxOneshotTransfer;
		...
		fn can_distribute_once() {
			new_test_ext().execute_with(|| {
				set_block_1();
				assert_ok!(cyruxOneshotTransfer::distribute(
					Origin::signed(1),
				...
				assert_eq!(
					...
					vec![
						...
						TestEvent::cyruxOneshotTransfer(Event::AccountsBlacklisted(vec![2, 3, 1]))
					]
				);

				assert_noop!(
					cyruxOneshotTransfer::distribute(Origin::signed(1), vec![(4, DOLLARS)]),
					Error::<Test>::SenderAlreadyBlacklisted
				);
				assert_noop!(
					cyruxOneshotTransfer::distribute(
						Origin::signed(5),
		...
		fn no_enough_existence_deposit() {
			new_test_ext().execute_with(|| {
				set_block_1();
				assert_noop!(
					cyruxOneshotTransfer::distribute(Origin::signed(1), vec![(10, 1)]),
