...
use cyrux_types::messaging::Message;
use sp_core::H256;
use sp_runtime::{
...
frame_support::construct_runtime!(
	pub struct Test {
		...
		cyruxComputation: computation::{Pallet, Event<T>, Storage, Config<T>},
		cyruxStakePoolv2: stake_pool_v2::{Pallet, Event<T>},
		cyruxVault: vault::{Pallet, Event<T>},
		cyruxWrappedBalances: wrapped_balances::{Pallet, Event<T>},
		cyruxBasePool: base_pool::{Pallet, Event<T>},
		cyruxStakePool: stake_pool::{Event<T>},
		Preimage: pallet_preimage::{Event<T>},
	}
);

impl crate::cyruxConfig for Test {
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Test> {
		balances: vec![
			...
			(cyruxComputation::account_id(), 690_000_000 * DOLLARS),
		],
	}
...
pub fn set_block_1() {
	...
	cyruxRegistry::internal_set_gk_launched_at(0, 0);
}
...
pub fn take_messages() -> Vec<Message> {
	let messages = cyruxMq::messages();
	mq::OutboundMessages::<Test>::kill();
	messages
}

use cyrux_types::{EcdhPublicKey, WorkerPublicKey};
...
pub fn setup_workers(n: u8) {
	...
	for i in 1..=n {
		...
		cyruxRegistry::internal_set_benchmark(&worker, Some(1));
	}
}
...
pub fn elapse_cool_down() {
	let now = Timestamp::get();
	Timestamp::set_timestamp(now + cyruxComputation::cool_down_period() * 1000);
}
