...
pub mod pallet {
	...
	where
		...
		T: pallet_democracy::Config<Currency = <T as crate::cyruxConfig>::Currency>,
		...
		T: Config + vault::Config,
	{
		...
		pub fn wrap(origin: OriginFor<T>, amount: BalanceOf<T>) -> DispatchResult {
			...
			<T as cyruxConfig>::Currency::transfer(
				&user,
		...
		T: Config + vault::Config,
	{
		...
		pub fn remove_dust(who: &T::AccountId, dust: BalanceOf<T>) {
			...
			if dust != Zero::zero() {
				...
				let (imbalance, _remaining) = <T as cyruxConfig>::Currency::slash(
