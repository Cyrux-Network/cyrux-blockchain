...
pub mod pallet {
	...
	use crate::registry;
	use crate::{BalanceOf, NegativeImbalanceOf, cyruxConfig};
	...
	const COMPUTING_PALLETID: PalletId = PalletId(*b"cyrux/pp");
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + cyruxConfig + mq::Config + registry::Config {
		...
		BalanceOf<T>: FixedPointConvert,
	{
		...
		pub fn withdraw_subsidy_pool(target: &T::AccountId, value: BalanceOf<T>) -> DispatchResult {
			...
			<T as cyruxConfig>::Currency::transfer(&wallet, target, value, KeepAlive)
		}
		...
		pub fn withdraw_imbalance_from_subsidy_pool(
			...
			let wallet = Self::account_id();
			<T as cyruxConfig>::Currency::withdraw(
				&wallet,
				value,
	...
	impl<T: Config> Default for GenesisConfig<T> {
		/// Default tokenomic parameters for cyrux
		fn default() -> Self {
			use fixed_macro::types::U64F64 as fp;
			let block_sec = 12;
			let hour_sec = 3600;
	...
	mod test {
		...
		// Pallets
		use crate::mock::{cyruxComputation, cyruxRegistry, System};
		...
		fn test_computing_wallet_setup() {
			new_test_ext().execute_with(|| {
				assert!(System::account_exists(&cyruxComputation::account_id()));
			});
		}
		...
		fn test_heartbeat_challenge() {
			new_test_ext().execute_with(|| {
				use cyrux_types::messaging::{SystemEvent, Topic};
				...
				assert_eq!(message.destination, Topic::new("cyrux/system/event"));
		...
		fn test_bind_unbind() {
			new_test_ext().execute_with(|| {
				...
				let _ = take_events();
				assert_ok!(cyruxComputation::bind(1, worker_pubkey(1)));
				assert_ok!(cyruxComputation::unbind_session(&1, false));
				assert_eq!(
					take_events().as_slice(),
					[
						TestEvent::cyruxComputation(Event::SessionBound {
							session: 1,
							worker: worker_pubkey(1)
						}),
						TestEvent::cyruxComputation(Event::SessionUnbound {
							session: 1,
							worker: worker_pubkey(1)
						})
					]
				...
				// Checks edge cases
				assert_noop!(
					cyruxComputation::bind(1, worker_pubkey(100)),
					Error::<Test>::WorkerNotRegistered,
				);
				assert_noop!(
					cyruxComputation::unbind_session(&1, false),
					Error::<Test>::SessionNotBound,
				);
				// No double binding
				assert_ok!(cyruxComputation::bind(2, worker_pubkey(2)));
				assert_noop!(
					cyruxComputation::bind(2, worker_pubkey(1)),
					Error::<Test>::DuplicateBoundSession
				);
				assert_noop!(
					cyruxComputation::bind(1, worker_pubkey(2)),
		...
		fn test_stakepool_callback_panic() {
			new_test_ext().execute_with(|| {
				...
				setup_workers(1);
				assert_ok!(cyruxComputation::bind(1, worker_pubkey(1)));
				assert_ok!(cyruxComputation::start_computing(1, 1000 * DOLLARS));
				// Force unbind without StakePool registration will cause a panic
				let _ = cyruxComputation::unbind(Origin::signed(1), 1);
			});
		}
		...
		fn tokenomic_update_is_postponed() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(cyruxComputation::update_tokenomic(
					Origin::root(),
					tokenomic
				));
				let ev = take_events();
				assert_eq!(ev.len(), 0);
				cyruxComputation::on_finalize(1);
				let ev = take_events();
				assert_eq!(ev.len(), 1);
		...
		fn test_benchmark_report() {
			...
			new_test_ext().execute_with(|| {
				...
				setup_workers(1);
				// 100 iters per sec
				cyruxRegistry::internal_set_benchmark(&worker_pubkey(1), Some(600));
				assert_ok!(cyruxComputation::bind(1, worker_pubkey(1)));
				assert_ok!(cyruxComputation::start_computing(1, 3000 * DOLLARS));
				...
				// 110% boost
				elapse_seconds(100);
				assert_ok!(cyruxComputation::on_working_message_received(
					DecodedMessage::<WorkingReportEvent> {
						sender: MessageOrigin::Worker(worker_pubkey(1)),
						destination: Topic::new(*b"cyrux/mining/report"),
				...
				let worker = cyruxComputation::sessions(1).unwrap();
				assert_eq!(
					worker.benchmark,
					Benchmark {
						p_init: 600,
		...
		fn drop_late_arrived_update() {
			new_test_ext().execute_with(|| {
				use cyrux_types::messaging::Topic;
				...
				assert_ok!(cyruxComputation::start_computing(1, 3000 * DOLLARS));
				take_events();
				assert_ok!(cyruxComputation::on_gk_message_received(DecodedMessage::<
					WorkingInfoUpdateEvent<BlockNumber>,
				> {
					sender: MessageOrigin::Gatekeeper,
					destination: Topic::new(*b"^cyrux/mining/update"),
				...
				assert_eq!(
					ev[0],
					TestEvent::cyruxComputation(Event::SessionSettlementDropped {
						session: 1,
						v: 1,
						payout: 0,
					})
