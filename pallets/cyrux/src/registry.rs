...
pub mod pallet {
	...
	pub use cyrux_types::AttestationReport;
	// Re-export
	// TODO: Legacy
	...
	bind_topic!(RegistryEvent, b"^cyrux/registry/event");
	...
	bind_topic!(GatekeeperRegistryEvent, b"^cyrux/registry/gk_event");
	#[derive(Encode, Decode, TypeInfo, Clone, Debug)]
	...
	/// Mapping from worker pubkey to Cyrux Network identity
	#[pallet::storage]
	...
	pub struct OldWorkerInfo<AccountId> {
		...
		/// The [confidence level](https://wiki.cyrux.network/en-us/mine/solo/1-2-confidential-level-evaluation/#confidence-level-of-a-miner)
		/// of the worker
	...
	mod test {
		...
		fn test_register_worker() {
			new_test_ext().execute_with(|| {
				...
				// New registration
				assert_ok!(cyruxRegistry::register_worker(
					Origin::signed(1),
					WorkerRegistrationInfo::<u64> {
						version: 1,
						machine_id: Default::default(),
		...
		fn test_register_worker_v2() {
			new_test_ext().execute_with(|| {
				...
				// New registration without valid genesis_block_hash
				assert_noop!(
					cyruxRegistry::register_worker_v2(
						Origin::signed(1),
				...
				// New registration
				assert_ok!(cyruxRegistry::register_worker_v2(
					Origin::signed(1),
					WorkerRegistrationInfoV2::<u64> {
						version: 1,
						machine_id: Default::default(),
		...
		fn test_pruntime_allowlist_works() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(cyruxRegistry::add_pruntime(Origin::root(), sample.clone()));
				assert_noop!(
					cyruxRegistry::add_pruntime(Origin::root(), sample.clone()),
				...
				assert_ok!(cyruxRegistry::remove_pruntime(
					Origin::root(),
					sample.clone()
				));
				assert_noop!(
					cyruxRegistry::remove_pruntime(Origin::root(), sample.clone()),
		...
		fn test_relaychain_genesis_block_hash_allowlist_works() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(cyruxRegistry::add_relaychain_genesis_block_hash(
					Origin::root(),
					sample
				));
				assert_noop!(
					cyruxRegistry::add_relaychain_genesis_block_hash(Origin::root(), sample),
					Error::<Test>::GenesisBlockHashAlreadyExists
				);
				assert_eq!(RelaychainGenesisBlockHashAllowList::<Test>::get().len(), 1);
				assert_ok!(cyruxRegistry::remove_relaychain_genesis_block_hash(
					Origin::root(),
					sample
				));
				assert_noop!(
					cyruxRegistry::remove_relaychain_genesis_block_hash(Origin::root(), sample),
