...
// Pallets
use crate::mock::{
	Balances, cyruxBasePool, cyruxComputation, cyruxRegistry, cyruxStakePoolv2, cyruxVault,
	cyruxWrappedBalances, Preimage, RuntimeCall,
};
use pallet_democracy::AccountVote;
use pallet_democracy::BoundedCallOf;
use cyrux_types::{messaging::SettleInfo, WorkerPublicKey};
...
fn test_wrap() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxWrappedBalances::wrap(
...
fn test_unwrap() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxWrappedBalances::unwrap(
			RuntimeOrigin::signed(1),
			50 * DOLLARS,
		));
		let free = Balances::free_balance(1);
		...
		assert_noop!(
			cyruxWrappedBalances::unwrap(RuntimeOrigin::signed(1), 50 * DOLLARS),
...
fn test_unwrap_all() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxStakePoolv2::contribute(
			RuntimeOrigin::signed(1),
			0,
			50 * DOLLARS,
			None
		...
		assert_ok!(cyruxWrappedBalances::unwrap_all(RuntimeOrigin::signed(1)));
		let free = Balances::free_balance(1);
		assert_eq!(free, 950 * DOLLARS);
		assert_ok!(cyruxWrappedBalances::wrap(
		...
		assert_ok!(cyruxWrappedBalances::unwrap_all(RuntimeOrigin::signed(2)));
		let free = Balances::free_balance(2);
		assert_eq!(free, 2000 * DOLLARS);
	});
}
...
fn test_vote() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			cyruxWrappedBalances::vote(RuntimeOrigin::signed(1), 90 * DOLLARS, 90 * DOLLARS, 0),
			wrapped_balances::Error::<Test>::VoteAmountLargerThanTotalStakes,
		);
		assert_ok!(cyruxWrappedBalances::vote(
			RuntimeOrigin::signed(1),
			20 * DOLLARS,
			10 * DOLLARS,
			0
		...
		let vote = cyruxWrappedBalances::accumulate_account_vote(0);
...
fn test_unlock() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			cyruxWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 1),
			wrapped_balances::Error::<Test>::ReferendumOngoing,
		);
		pallet_democracy::pallet::Pallet::<Test>::internal_cancel_referendum(0);
		assert_ok!(cyruxWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 1));
		let account1_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(1).unwrap();
		...
		assert_ok!(cyruxWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 2));
		let account2_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(2).unwrap();
		...
		assert_ok!(cyruxWrappedBalances::unlock(RuntimeOrigin::signed(3), 1, 2));
		let account1_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(1).unwrap();
		assert_eq!(account1_status.locked, 0);
		let account2_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(2).unwrap();
		assert_eq!(account2_status.locked, 0);
...
fn test_mint_nft() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxBasePool::mint_nft(
		...
		{
			assert_ok!(cyruxBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0));
			assert_noop!(
				cyruxBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0),
				base_pool::Error::<Test>::AttrLocked
			);
		}
		let nft_attr = cyruxBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0)
			.unwrap()
			.attr
			.clone();
		assert_eq!(nft_attr.shares, 1000 * DOLLARS);
		let nft_attr = cyruxBasePool::get_nft_attr_guard(pool_info.basepool.cid, 1)
			.unwrap()
			.attr
			.clone();
...
fn test_merge_nft() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxBasePool::merge_nft_for_staker(
			pool_info.basepool.cid,
			1,
			pool_info.basepool.pid,
		));
		...
		{
			let nft_attr = cyruxBasePool::get_nft_attr_guard(pool_info.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
	...
	new_test_ext().execute_with(|| {
		...
		));
		{
			let mut nft_attr_guard =
				cyruxBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0).unwrap();
...
fn test_remove_stake_from_nft() {
	new_test_ext().execute_with(|| {
		...
		let mut nft_attr = cyruxBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
			.unwrap()
			.attr
			.clone();
		assert_eq!(pool.basepool.share_price().unwrap(), 1);
		match cyruxBasePool::remove_stake_from_nft(
			&mut pool.basepool,
			40 * DOLLARS,
			&mut nft_attr,
			&1,
...
fn test_create_stakepool() {
	new_test_ext().execute_with(|| {
		set_block_1();
		assert_ok!(cyruxStakePoolv2::create(RuntimeOrigin::signed(1)));
		assert_ok!(cyruxStakePoolv2::create(RuntimeOrigin::signed(2)));
		assert_eq!(
...
fn test_create_vault() {
	new_test_ext().execute_with(|| {
		set_block_1();
		assert_ok!(cyruxVault::create(RuntimeOrigin::signed(1)));
		assert_ok!(cyruxVault::create(RuntimeOrigin::signed(2)));
		assert_eq!(
			base_pool::Pools::<Test>::get(0),
...
fn test_contribute() {
	new_test_ext().execute_with(|| {
		...
		setup_stake_pool_with_workers(1, &[1, 2]); // pid = 0
		assert_noop!(
			cyruxStakePoolv2::contribute(RuntimeOrigin::signed(1), 0, 50 * DOLLARS, None,),
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let nft_attr = cyruxBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
		...
		assert_ok!(cyruxVault::create(RuntimeOrigin::signed(1)));
		assert_ok!(cyruxVault::contribute(
			RuntimeOrigin::signed(2),
			1,
			200 * DOLLARS,
		));
		...
		assert_noop!(
			cyruxStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 10 * DOLLARS, Some(1),),
...
fn test_set_pool_description() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxBasePool::set_pool_description(
			RuntimeOrigin::signed(1),
			0,
			str_hello.clone(),
		));
		let list = cyruxBasePool::pool_descriptions(0).unwrap();
		assert_eq!(list, str_hello);
		let str_bye: base_pool::DescStr = ("bye").as_bytes().to_vec().try_into().unwrap();
		assert_noop!(
			cyruxBasePool::set_pool_description(RuntimeOrigin::signed(2), 0, str_bye,),
...
fn test_staker_whitelist() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxBasePool::add_staker_to_whitelist(
			RuntimeOrigin::signed(1),
			0,
			2,
		));
		let whitelist = cyruxBasePool::pool_whitelist(0).unwrap();
		...
		assert_noop!(
			cyruxStakePoolv2::contribute(RuntimeOrigin::signed(3), 0, 40 * DOLLARS, None),
		...
		assert_ok!(cyruxBasePool::remove_staker_from_whitelist(
			RuntimeOrigin::signed(1),
			0,
			2
		));
		...
		assert_noop!(
			cyruxStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 20 * DOLLARS, None),
			base_pool::Error::<Test>::NotInContributeWhitelist
		);
		assert_ok!(cyruxBasePool::remove_staker_from_whitelist(
			RuntimeOrigin::signed(1),
		...
		assert!(cyruxBasePool::pool_whitelist(0).is_none());
		assert_ok!(cyruxStakePoolv2::contribute(
			RuntimeOrigin::signed(3),
			0,
			20 * DOLLARS,
...
fn test_pool_cap() {
	new_test_ext().execute_with(|| {
		...
		// Pool existence
		assert_noop!(
			cyruxStakePoolv2::set_cap(RuntimeOrigin::signed(2), 100, 1),
			base_pool::Error::<Test>::PoolDoesNotExist,
		);
		// Owner only
		assert_noop!(
			cyruxStakePoolv2::set_cap(RuntimeOrigin::signed(2), 0, 1),
			stake_pool_v2::Error::<Test>::UnauthorizedPoolOwner,
		);
		// Cap to 1000 PHA
		assert_ok!(cyruxStakePoolv2::set_cap(
			RuntimeOrigin::signed(1),
			0,
			100 * DOLLARS
		));
		...
		assert_noop!(
			cyruxStakePoolv2::set_cap(RuntimeOrigin::signed(1), 0, 9 * DOLLARS),
		...
		// Exceed the cap
		assert_noop!(
			cyruxStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 90 * DOLLARS, None),
		...
		assert_ok!(cyruxStakePoolv2::start_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
			100 * DOLLARS
		));
		assert_ok!(cyruxStakePoolv2::withdraw(
			RuntimeOrigin::signed(1),
			0,
			100 * DOLLARS,
			None
		...
		assert_noop!(
			cyruxStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 101 * DOLLARS, None),
			stake_pool_v2::Error::<Test>::StakeExceedsCapacity
		);
		assert_ok!(cyruxStakePoolv2::contribute(
			RuntimeOrigin::signed(2),
...
fn test_add_worker() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxRegistry::force_register_worker(
			RuntimeOrigin::root(),
			worker1,
			ecdh_pubkey(1),
			Some(1)
		...
		// Bad inputs
		assert_noop!(
			cyruxStakePoolv2::add_worker(RuntimeOrigin::signed(1), 1, worker2),
			stake_pool_v2::Error::<Test>::WorkerNotRegistered
		);
		assert_noop!(
			cyruxStakePoolv2::add_worker(RuntimeOrigin::signed(2), 0, worker1),
			stake_pool_v2::Error::<Test>::UnauthorizedOperator
		);
		assert_noop!(
			cyruxStakePoolv2::add_worker(RuntimeOrigin::signed(1), 0, worker1),
			stake_pool_v2::Error::<Test>::BenchmarkMissing
		);
		// Add benchmark and retry
		cyruxRegistry::internal_set_benchmark(&worker1, Some(1));
		assert_ok!(cyruxStakePoolv2::add_worker(
			RuntimeOrigin::signed(1),
			0,
			worker1
		));
		...
		assert_eq!(
			cyruxComputation::ensure_worker_bound(&worker_pubkey(1)).unwrap(),
			subaccount,
		);
		assert_eq!(
			cyruxComputation::ensure_session_bound(&subaccount).unwrap(),
			worker_pubkey(1),
		);
		// Check assignments
		assert_eq!(
		...
		// Other bad cases
		assert_noop!(
			cyruxStakePoolv2::add_worker(RuntimeOrigin::signed(1), 100, worker1),
			base_pool::Error::<Test>::PoolDoesNotExist
		);
		// Bind one worker to antoher pool (pid = 1)
		assert_ok!(cyruxStakePoolv2::create(RuntimeOrigin::signed(1)));
		assert_noop!(
			cyruxStakePoolv2::add_worker(RuntimeOrigin::signed(1), 1, worker1),
...
fn test_start_computing() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			cyruxStakePoolv2::start_computing(RuntimeOrigin::signed(1), 0, worker_pubkey(1), 0),
		...
		assert_noop!(
			cyruxStakePoolv2::start_computing(
				RuntimeOrigin::signed(1),
				0,
				worker_pubkey(1),
				30000 * DOLLARS
		...
		assert_eq!(cyruxComputation::online_workers(), 1);
		let pool = ensure_stake_pool::<Test>(0).unwrap();
...
fn test_force_unbind() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxComputation::unbind(
		...
		let worker = cyruxComputation::sessions(sub_account).unwrap();
...
fn test_stop_computing() {
	new_test_ext().execute_with(|| {
		...
		let lock = get_balance(pool.lock_account);
		assert_eq!((balance, lock), (0, 100 * DOLLARS));
		assert_ok!(cyruxStakePoolv2::stop_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
		));
...
fn test_reclaim() {
	new_test_ext().execute_with(|| {
		...
		elapse_cool_down();
		assert_ok!(cyruxStakePoolv2::reclaim_pool_worker(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
		));
...
fn restart_computing_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Bad cases
		assert_noop!(
			cyruxStakePoolv2::restart_computing(
				RuntimeOrigin::signed(1),
				0,
				worker_pubkey(1),
				50 * DOLLARS
		...
		assert_ok!(cyruxStakePoolv2::restart_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
			151 * DOLLARS
...
fn test_for_cdworkers() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			cyruxStakePoolv2::remove_worker(RuntimeOrigin::signed(1), 0, worker_pubkey(1)),
		...
		assert_ok!(cyruxStakePoolv2::remove_worker(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
		));
...
fn test_on_reward_dust() {
	...
	new_test_ext().execute_with(|| {
		...
		setup_workers(1);
		setup_stake_pool_with_workers(1, &[1]);
		assert_ok!(cyruxStakePoolv2::set_payout_pref(
			RuntimeOrigin::signed(1),
			0,
		...
		let _ = take_events();
		cyruxStakePoolv2::on_reward(&[SettleInfo {
...
fn test_claim_owner_rewards() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(cyruxStakePoolv2::claim_owner_rewards(
			RuntimeOrigin::signed(1),
			0,
			1
		));
...
fn test_vault_owner_shares() {
	...
	new_test_ext().execute_with(|| {
		...
		setup_workers(1);
		setup_vault(3); // pid = 0
		assert_ok!(cyruxVault::set_payout_pref(
			RuntimeOrigin::signed(3),
			0,
			Some(Permill::from_percent(50))
		));
		...
		assert_ok!(cyruxVault::maybe_gain_owner_shares(
			RuntimeOrigin::signed(3),
			0
		));
		let vault_info = ensure_vault::<Test>(0).unwrap();
		...
		assert_noop!(
			cyruxVault::claim_owner_shares(RuntimeOrigin::signed(3), 0, 4, 50 * DOLLARS),
			vault::Error::<Test>::NoEnoughShareToClaim
		);
		assert_ok!(cyruxVault::claim_owner_shares(
			RuntimeOrigin::signed(3),
			0,
			4,
			10 * DOLLARS
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let nft_attr = cyruxBasePool::get_nft_attr_guard(vault_info.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
...
fn test_withdraw() {
	new_test_ext().execute_with(|| {
		...
		{
			let nft_attr =
				cyruxBasePool::get_nft_attr_guard(pool.basepool.cid, item.unwrap().nft_id)
					.unwrap()
					.attr
					.clone();
		...
		let _ = take_events();
		assert_ok!(cyruxVault::check_and_maybe_force_withdraw(
...
fn test_check_and_maybe_force_withdraw() {
	new_test_ext().execute_with(|| {
		...
		assert_user_has_share(10000, 2, 0);
		assert_eq!(get_balance(2), 400 * DOLLARS);
		assert_ok!(cyruxStakePoolv2::check_and_maybe_force_withdraw(
			RuntimeOrigin::signed(3),
			0
		));
		let pool = ensure_stake_pool::<Test>(0).unwrap();
		...
		{
			let nft_attr =
				cyruxBasePool::get_nft_attr_guard(vault.basepool.cid, item.unwrap().nft_id)
					.unwrap()
					.attr
					.clone();
		...
		assert_noop!(
			cyruxStakePoolv2::withdraw(RuntimeOrigin::signed(99), 0, 100 * DOLLARS, Some(pid)),
...
fn setup_stake_pool_with_workers(owner: u64, workers: &[u8]) -> u64 {
	let pid = cyruxBasePool::pool_count();
	assert_ok!(cyruxStakePoolv2::create(RuntimeOrigin::signed(owner)));
	for id in workers {
		assert_ok!(cyruxStakePoolv2::add_worker(
...
fn setup_vault(owner: u64) -> u64 {
	let pid = cyruxBasePool::pool_count();
	assert_ok!(cyruxVault::create(RuntimeOrigin::signed(owner)));
	pid
}
...
fn assert_user_has_share(cid: u32, owner: u64, shares: u128) {
	...
	assert_eq!(
		cyruxBasePool::get_nft_attr_guard(cid, nftid_arr[0])
			.unwrap()
			.attr
			.shares,
		shares
