...
pub fn tag(sender: &MessageOrigin, seq: u64) -> Vec<u8> {
	("cyruxMqOffchainMessages", sender, seq).encode()
}
...
mod tests {
	...
	use cyrux_types::messaging::{Message, MessageOrigin, SignedMessage, Topic};
	...
	fn sync_msg_call(i: u8, seq: u64) -> TestCall {
		TestCall::cyruxMq(Call::<Test>::sync_offchain_message {
