...
const pathNode = path.resolve('../target/release/cyrux-node');
...
describe('A full stack', function () {
    ...
    describe('pRuntime', () => {
        ...
        it('finishes the benchmark', async function () {
            ...
            assertTrue(await checkUntil(async () => {
                const workerInfo = await api.query.cyruxRegistry.workers(workerKey);
    ...
    describe('Gatekeeper', () => {
        ...
        it('can be registered as first gatekeeper', async function () {
            ...
            await assert.txAccepted(
                api.tx.sudo.sudo(
                    api.tx.cyruxRegistry.registerGatekeeper(hex(info.system.publicKey))
                ),
                alice,
            );
            // Finalization takes 2-3 blocks. So we wait for 3 blocks here.
            ...
            assertTrue(await checkUntil(async () => {
                const info = await pruntime[0].getInfo();
                const gatekeepers = await api.query.cyruxRegistry.gatekeeper();
        ...
        it('finishes master pubkey upload', async function () {
            assertTrue(await checkUntil(async () => {
                const masterPubkey = await api.query.cyruxRegistry.gatekeeperMasterPubkey();
                return masterPubkey.isSome;
            }, 4 * 6000), 'master pubkey not uploaded');
            const launchedAt = await api.query.cyruxRegistry.gatekeeperLaunchedAt();
    ...
    describe.optional('Gatekeeper2', () => {
        ...
        it('post-mines blocks', async function () {
            ...
            let seqStart = await api.query.cyruxMq.offchainIngress(gatekeeper);
            seqStart = seqStart.unwrap().toNumber();
            assertTrue(await checkUntil(async () => {
                let seq = await api.query.cyruxMq.offchainIngress(gatekeeper);
        ...
        it('can be unregistered', async function () {
            ...
            await assert.txAccepted(
                api.tx.sudo.sudo(
                    api.tx.cyruxRegistry.unregisterGatekeeper(hex(info.system.publicKey))
                ),
                alice,
            );
    ...
    describe.optional('Master Key Rotation', () => {
        ...
        it('can rotate master key', async function () {
            ...
            await assert.txAccepted(
                api.tx.sudo.sudo(
                    api.tx.cyruxRegistry.rotateMasterKey()
                ),
                alice,
            );
    ...
    describe('Cluster & Contract', () => {
        ...
        before(async () => {
            certAlice = await cyrux.signCertificate({ api, pair: alice });
            certBob = await cyrux.signCertificate({ api, pair: bob });
        });
        ...
        it('can upload system code', async function () {
            ...
            await assert.txAccepted(
                api.tx.sudo.sudo(api.tx.cyruxPhatContracts.setPinkSystemCode(systemCode)),
                alice,
            );
            assertTrue(await checkUntil(async () => {
                let code = await api.query.cyruxPhatContracts.pinkSystemCode();
        ...
        it('can create cluster', async function () {
            ...
            await assert.txAccepted(
                api.tx.sudo.sudo(
                    api.tx.cyruxPhatContracts.addCluster(
                        alice.address,
                        perm,
            ...
            assertTrue(await checkUntil(async () => {
                const clusters = await api.query.cyruxPhatContracts.clusters.entries();
            ...
            const clusterInfo = await api.query.cyruxPhatContracts.clusters(clusterId);
            const { systemContract } = clusterInfo.unwrap();
            assertTrue(await checkUntil(async () => {
                const contractInfo = await api.query.cyruxPhatContracts.contracts(systemContract.toHex());
            ...
            registry = await cyrux.OnChainRegistry.create(api, { clusterId, pruntimeURL: pruntime[0].uri, systemContractId, workerId: runtime0.system.publicKey, skipCheck: true })
            ...
            ContractSystem = new cyrux.PinkContractPromise(api, registry, systemMetadata, systemContractId, contractKey)
        });

        it('can generate cluster key', async function () {
            assertTrue(await checkUntil(async () => {
                const clusterKey = await api.query.cyruxRegistry.clusterKeys(clusterId);
        ...
        it('can deploy cluster to multiple workers', async function () {
            assertTrue(await checkUntil(async () => {
                const clusterWorkers = await api.query.cyruxPhatContracts.clusterWorkers(clusterId);
        ...
        it('can upload code with access control', async function () {
            ...
            await assert.txAccepted(
                api.tx.cyruxPhatContracts.clusterUploadResource(clusterId, 'InkCode', hex(code)),
                alice,
            );
        ...
        async function deployContract(metadata) {
            ...
            await assert.txAccepted(
                api.tx.cyruxPhatContracts.clusterUploadResource(clusterId, 'InkCode', code),
                alice,
            );
            assertTrue(await checkUntil(async () => {
                const { output } = await ContractSystem.query['system::codeExists'](alice.address, { cert: certAlice }, codeHash, 'Ink');
            ...
            const salt = cyrux.randomHex(8);
            const { events } = await assert.txAccepted(
                api.tx.cyruxPhatContracts.instantiateContract(codeIndex, selectorDefault, salt, clusterId, 0, "10000000000000", null, 0),
                alice,
            );
            assertEvents(events, [
                ['balances', 'Withdraw'],
                ['cyruxPhatContracts', 'Instantiating']
            ]);

            const { event } = events[1];
            let contractId = hex(event.toJSON().data[0]);
            let contractInfo = await api.query.cyruxPhatContracts.contracts(contractId);
            assertTrue(contractInfo.isSome, 'no contract info');

            assertTrue(await checkUntil(async () => {
                let key = await api.query.cyruxRegistry.contractKeys(contractId);
                return key.isSome;
            }, 4 * 6000), 'contract key generation failed');

            assertTrue(await checkUntil(async () => {
                let clusterContracts = await api.query.cyruxPhatContracts.clusterContracts(clusterId);
            ...
            return new cyrux.PinkContractPromise(api, registry, metadata, contractId, contractKey);
        }
        ...
        it.optional('can not set hook without admin permission', async function () {
            ...
            await assert.txAccepted(
                api.tx.utility.batchAll([
                    api.tx.cyruxPhatContracts.transferToCluster(CENTS * 1000, clusterId, ContractSystemChecker.address),
        ...
        it.optional('can eval JavaScript in contract delegate call', async function () {
            ...
            await assert.txAccepted(
                api.tx.cyruxPhatContracts.clusterUploadResource(clusterId, 'IndeterministicInkCode', hex(code)),
                alice,
            );
            assertTrue(await checkUntil(async () => {
                const { output } = await ContractSystem.query['system::codeExists'](alice.address, { cert: certAlice }, codeHash, 'Ink');
        ...
        it.optional('can eval JavaScript with JsRuntime running in SideVM', async function () {
            await assert.txAccepted(
                api.tx.cyruxPhatContracts.clusterUploadResource(clusterId, 'SidevmCode', jsRuntimeCode),
                alice,
            );
            await assert.txAccepted(
        ...
        it.optional('tokenomic driver works', async function () {
            ...
            const weight = 10;
            await assert.txAccepted(
                api.tx.cyruxPhatTokenomic.adjustStake(ContractSystemChecker.address, weight * CENTS),
                alice,
            );
        ...
        it.optional('cannot dup-instantiate', async function () {
            const codeIndex = api.createType('CodeIndex', { 'WasmCode': codeHash });
            await assert.txFailed(
                api.tx.cyruxPhatContracts.instantiateContract(codeIndex, selectorDefault, 0, clusterId, 0, "10000000000000", null, 0),
                alice,
            );
        });
        ...
        it.optional('can deploy paid sidevm checkers', async function () {
            ...
            await assert.txAccepted(
                api.tx.utility.batchAll(
                    paidSidevmCheckers.map(checker => api.tx.cyruxPhatContracts.transferToCluster(CENTS * 10000, clusterId, checker.address))
                ),
                alice,
            );
            await syncBarrier();
        ...
        it.optional('can add worker to cluster', async function () {
            ...
            const { events } = await assert.txAccepted(
                api.tx.cyruxPhatContracts.addWorkerToCluster(hex(info.system?.publicKey), hex(clusterId)),
                alice,
            );
            assertEvents(events, [
                ['balances', 'Withdraw'],
                ['cyruxPhatContracts', 'WorkerAddedToCluster']
            ]);
            assertTrue(await checkUntil(async () => {
                const workerCluster = await api.query.cyruxPhatContracts.clusterByWorkers(hex(info.system?.publicKey));
        ...
        it('can destory cluster', async function () {
            await assert.txAccepted(
                api.tx.sudo.sudo(api.tx.cyruxPhatContracts.clusterDestroy(clusterId)),
                alice,
            );
            assertTrue(await checkUntil(async () => {
    ...
    describe.skip('Solo mining workflow', () => {
        ...
        it.skip('can bind the worker', async function () {
            const workerInfo = await api.query.cyruxRegistry.worker(workerKey);
            const operator = workerInfo.unwrap().operator.unwrap();
            assert.equal(operator.toHuman(), alice.address, 'bad operator');

            await assert.txAccepted(api.tx.cyruxMining.bind(workerKey), alice);

            let actualWorker = await api.query.cyruxMining.minerBinding(alice.address);
            let actualMiner = await api.query.cyruxMining.workerBinding(workerKey);
            ...
            let minerInfo = await api.query.cyruxMining.miners(alice.address);
        ...
        it.skip('can start mining', async function () {
            await assert.txAccepted(api.tx.cyruxMining.startMining(), alice);
            let minerInfo = await api.query.cyruxMining.miners(alice.address);
            minerInfo = minerInfo.unwrap();
...
function testPruntimeManagement(workDir) {
    describe.optional("PRuntime management", function () {
        ...
        it("can retire old pruntimes", async () => {
            await assert.txAccepted(
                api.tx.sudo.sudo(
                    api.tx.cyruxRegistry.setMinimumPruntimeVersion(
                        999,
                        0,
                        0,
                    )
        ...
        it("can not set consensus version over max supported", async () => {
            ...
            await assert.txAccepted(
                api.tx.sudo.sudo(
                    api.tx.cyruxRegistry.setPruntimeConsensusVersion(
...
class Cluster {
    ...
    async _createApi() {
        this.api = await ApiPromise.create({
            ...
            types: { ...types, ...typeDefinitions, ...cyrux.types, ...typeOverrides },
            typeAlias
        });
...
function newNode(rpcPort, tmpPath, name = 'node') {
    const cli = [
        pathNode, [
            ...
            '--base-path=' + path.resolve(tmpPath, 'cyrux-node'),
            `--rpc-port=${rpcPort}`,
            '--rpc-methods=Unsafe',
            '--pruning=archive',
        ]
...
async function createContractApi(api, pruntimeURL, contractId, metadata) {
    ...
    const cyrux = await cyrux.create({ api: newApi, baseURL: pruntimeURL, contractId, autoDeposit: true });
