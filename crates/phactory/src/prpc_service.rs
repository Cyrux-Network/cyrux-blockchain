...
use cyrux_crypto::ecdh::{self, EcdhPublicKey};
use cyrux_crypto::{
    key_share,
    sr25519::{Persistence, KDF},
};
use cyrux_mq::MessageOrigin;
use cyrux_pallets::utils::attestation::{validate as validate_attestation_report, IasFields};
use cyrux_types::contract::contract_id_preimage;
use cyrux_types::{
...
use cyrux_types::{DcapChallengeHandlerInfo, DcapHandoverChallenge};
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> Phactory<Platform> {
    ...
    fn init_runtime(
        ...
        // derive ecdh key
        let ecdh_pubkey = cyrux_types::EcdhPublicKey(ecdh_key.public());
    ...
    fn handle_inbound_messages(&mut self, block_number: chain::BlockNumber) -> RpcResult<()> {
        ...
        for message in messages {
            use cyrux_types::messaging::SystemEvent;
            macro_rules! log_message {
                ($msg: expr, $t: ident) => {{
                    ...
                    match event {
                        Ok(event) => {
                            debug!(target: "cyrux_mq",
                                "mq dispatching message: sender={} dest={:?} payload={:?}",
                                $msg.sender, $msg.destination, event
                            );
                        }
                        Err(_) => {
                            debug!(target: "cyrux_mq", "mq dispatching message (decode failed): {:?}", $msg);
                        }
                    }
                }};
            }
    ...
    fn load_cluster_state(&mut self, filename: &str) -> RpcResult<()> {
        ...
        let cluster_state: ClusterState =
            cyrux_mq::checkpoint_helper::using_dispatcher(recv_mq, move || {
                cyrux_mq::checkpoint_helper::using_send_mq(send_mq, move || {
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> PhactoryApi for RpcService<Platform> {
    ...
    async fn derive_cyrux_i2p_key(&mut self, _: ()) -> RpcResult<pb::DerivecyruxI2pKeyResponse> {
        let mut phactory = self.lock_phactory(true, false)?;
        let system = phactory.system()?;
        let derive_key = system
            .identity_key
            .derive_sr25519_pair(&[b"cyruxI2PKey"])
            .expect("should not fail with valid key");
        Ok(pb::DerivecyruxI2pKeyResponse {
            cyrux_i2p_key: derive_key.dump_secret_key().to_vec(),
        })
    }
    ...
    async fn handover_accept_challenge(
        ...
        let ecdh_pubkey = cyrux_types::EcdhPublicKey(handover_ecdh_key.public());
