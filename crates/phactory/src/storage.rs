...
mod storage_ext {
    ...
    use cyrux_mq::{ContractClusterId, Message, MessageOrigin};
    ...
    impl ChainStorage {
        ...
        pub fn mq_messages(&self) -> Vec<Message> {
            self.execute_with(chain::cyruxMq::messages)
        }
        ...
        pub(crate) fn gatekeepers(&self) -> Vec<cyrux_types::WorkerPublicKey> {
            self.execute_with(pallet_registry::Gatekeeper::<chain::Runtime>::get)
        }

        pub(crate) fn is_worker_registered(&self, worker: &cyrux_types::WorkerPublicKey) -> bool {
        ...
        pub(crate) fn get_worker_cluster(
            &self,
            worker: &cyrux_types::WorkerPublicKey,
