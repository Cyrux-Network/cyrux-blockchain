...
use cyrux_types::{VersionedWorkerEndpoints, WorkerPublicKey};
...
impl ChainApi {
    ...
    pub async fn worker_registered_at(
        ...
        let address = subxt::dynamic::storage("cyruxRegistry", "Workers", vec![worker]);
        let registered = self
            .storage()
            .at(hash)
            .fetch(&address)
    ...
    pub async fn worker_added_at(&self, worker: &[u8]) -> Result<Option<BlockNumber>> {
        let worker = Value::from_bytes(worker);
        let address = subxt::dynamic::storage("cyruxRegistry", "WorkerAddedAt", vec![worker]);
        let Some(block) = self
            .storage()
            .at_latest()
            .await?
    ...
    pub async fn get_workers(&self, cluster_id: Hash) -> Result<Vec<WorkerPublicKey>> {
        let result = self
            .fetch("cyruxPhatContracts", "ClusterWorkers", Some(&cluster_id))
            .await?;
    ...
    pub async fn get_endpoints(&self, worker: &WorkerPublicKey) -> Result<Vec<String>> {
        let result = self
            .fetch("cyruxRegistry", "Endpoints", Some(worker))
            .await?;
