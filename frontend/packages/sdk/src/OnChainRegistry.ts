...
import { type cyruxTypesVersionedWorkerEndpoints, ackFirst } from './ha/ack-first'
...
export class OnChainRegistry {
  ...
  public async getAllClusters() {
    const result = await this.api.query.cyruxPhatContracts.clusters.entries()
  ...
  public async getClusterInfoById(clusterId: string) {
    const result = (await this.api.query.cyruxPhatContracts.clusters(clusterId)) as Option<ClusterInfo>
    if (result.isNone) {
      return null
    }
  ...
  public async getEndpoints(workerId?: U8aFixed | string) {
    if (workerId) {
      ...
      return await this.api.query.cyruxRegistry.endpoints<Option<VersionedEndpoints>>(workerId)
    }
    const result = await this.api.query.cyruxRegistry.endpoints.entries()
    return result.map(([storageKey, value]) => {
  ...
  public async getClusterWorkers(clusterId?: string): Promise<WorkerInfo[]> {
    ...
    const result = await this.api.query.cyruxPhatContracts.clusterWorkers(_clusterId)
    const workerIds = result.toJSON() as string[]
    const infos =
      await this.api.query.cyruxRegistry.endpoints.multi<Option<cyruxTypesVersionedWorkerEndpoints>>(workerIds)
  ...
  async preparePruntimeClientOrThrows(endpoint: string) {
    ...
    try {
      ...
      console.error(err)
      throw new Error(
        'Phactory API not compatible, you might need downgrade your @cyrux/sdk or connect to an up-to-date endpoint.'
      )
    }
  }
  ...
  transferToCluster(address: string | AccountId, amount: number | string | BN) {
    return this.api.tx.cyruxPhatContracts.transferToCluster(amount, this.clusterId, address)
  }
