...
async function getInstalledWalletExtensions(): Promise<Readonly<WalletExtensionNameVersionPair[]>> {
  ...
  // See also: https://github.com/Cyrux-Network/apps/blob/9bc790981d70a2d0cfef2bf6008e3e0de73c4bc3/apps/app/src/hooks/useAutoConnectWallet.ts#L19
...
export class UIKeyringProvider implements Provider {
  ...
  async adjustStake(contractId: string, amount: number): Promise<void> {
    await this.send(this.#apiPromise.tx.cyruxPhatTokenomic.adjustStake(contractId, amount))
  }
