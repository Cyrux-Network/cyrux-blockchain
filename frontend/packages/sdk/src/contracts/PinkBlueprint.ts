...
import { cyruxTypes } from '../options'
  ...
  async waitFinalized(timeout: number = 120_000) {
    ...
    if (this.isInBlock || this.isFinalized) {
      ...
      while (true) {
        ...
        const result1 = (await this.registry.api.query.cyruxPhatContracts.clusterContracts(
          this.registry.clusterId
        )) as unknown as Text[]
        const contractIds = result1.map((i) => i.toString())
        if (contractIds.indexOf(contractId) !== -1) {
          const result2 = (await this.registry.api.query.cyruxRegistry.contractKeys(
            contractId
...
export class PinkBlueprintPromise {
  ...
  constructor(
    ...
    this.codeHash = cyruxTypes.createType('Hash', codeHash)
  ...
  #deploy = (
    ...
    return this.api.tx.cyruxPhatContracts
      .instantiateContract(
        { WasmCode: codeHash },
  ...
  #estimateGas = (
    ...
    const inkQueryInternal = async (origin: string | AccountId | Uint8Array) => {
      ...
      if (response.result.isErr) {
        return cyruxTypes.createType<InkQueryError>('InkQueryError', response.result.asErr.toHex())
      }
      const result = cyruxTypes.createType<ContractInstantiateResult>(
      ...
      if (result.result.isErr) {
        ...
        if (err.isModule && err.asModule.index.toNumber() === 4) {
          const contractError = cyruxTypes.createType('ContractError', result.result.asErr.asModule.error)
