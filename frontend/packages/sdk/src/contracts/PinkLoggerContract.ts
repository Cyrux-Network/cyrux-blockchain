...
function postProcessLogRecord<TDecodedEvent extends DecodedEvent = DecodedEvent>(
  ...
  return messages.map((message) => {
    if (message.type === 'MessageOutput') {
      const execResult = cyruxTypes.createType<ContractExecResult>('ContractExecResult', hexToU8a(message.output))
      ...
      if (
        ...
        const err = cyruxTypes.createType('ContractError', execResult.result.asErr.asModule.error)
        output.result = {
          err: {
...
export function getTopicHash(topic: LiteralTopic): string {
  ...
  const encoded = cyruxTypes.createType(`(Vec<u8>, [u8; ${length}])`, [null, event]).toU8a()
...
export class PinkLoggerContractPromise {
  ...
  constructor(
    ...
    this.#address = cyruxTypes.createType('AccountId', contractId)
    this.#pair = pair
