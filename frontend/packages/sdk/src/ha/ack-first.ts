...
export interface cyruxTypesVersionedWorkerEndpoints extends Enum {
...
export function ackFirst() {
  return async function ackFirst(
    ...
    const workersQuery = await apiPromise.query.cyruxPhatContracts.clusterWorkers<Vec<U8aFixed>>(clusterId)
    const workerIds = workersQuery.map((i) => i.toHex())
    const endpointsQuery =
      await apiPromise.query.cyruxRegistry.endpoints.multi<Option<cyruxTypesVersionedWorkerEndpoints>>(workerIds)
