...
function CertificateBody(pubkey: string, ttl: number, config_bits: number = 0) {
  const created = cyruxTypes.createType('CertificateBody', { pubkey, ttl, config_bits })
...
export async function signCertificate(params: CertificateParams): Promise<CertificateData> {
  ...
  if (
    ...
    throw new Error(
      'signCertificate: invalid parameters. Please check document for more information: https://www.npmjs.com/package/@cyrux/sdk'
    )
  }
