...
export async function pinkQuery(
  ...
  const encodedEncryptedData = cyruxTypes.createType('EncryptedData', encryptedData).toU8a()
  ...
  const { data: encryptedResult, iv } = cyruxTypes.createType<IEncryptedData>('EncryptedData', res.encodedEncryptedData)
  const data = hexAddPrefix(decrypt(encryptedResult.toString(), agreement.agreementKey, iv))
  const resp = cyruxTypes.createType<InkResponse>('InkResponse', data)
