...
export type GetClientOptions = {
  ...
  // It's optional since if the RPC under the cyrux.network domain, we will
...
export async function getClient(opts: GetClientOptions): Promise<OnChainRegistry> {
  ...
  if (typeof transport === 'string' && !metadata && transport.indexOf('cyrux.network/') !== -1 && !noPreloadMetadata) {
