...
const khala = {
    ...
    // The pRuntime hashes, signed by cyrux's Intel SGX IAS key.
    pruntimeHashes: [
...
async function main() {
    ...
    const motion2 = api.tx.utility.batchAll([
        ...
        ... params.pruntimeHashes.map(h => api.tx.cyruxRegistry.addPruntime(h)),
        // 2. Register the whitelisted relayc chain genesis hashes
        ... params.relayGenesisHashes.map(([_n, blockHash]) =>
            api.tx.cyruxRegistry.addRelaychainGenesisBlockHash(blockHash)
        ),
    ]);
    ...
    const motion3 = api.tx.utility.batchAll([
        ...
        api.tx.cyruxRegistry.registerGatekeeper(params.genesisGatekeeper),
        ...
        api.tx.scheduler.schedule(
            ...
            api.tx.cyruxStakePool.setMiningEnable(true)
        ),
    ]);
    ...
    const motion4 = api.tx.cyruxRegistry.registerGatekeeper(params.secondGatekeeper);
