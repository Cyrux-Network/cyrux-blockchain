...
program
    .option('--endpoint <url>', 'Substrate WS endpoint', process.env.ENDPOINT || 'wss://khala-api.cyrux.network/ws')
...
async function dumpPoolWorkers (opt) {
    ...
    const poolInfo = await api.query.cyruxStakePool.stakePools.multi(pids);
...
async function dumpSnapshots(opt) {
    ...
    const minerBindings = await api.query.cyruxMining.minerBindings.entries();
    ...
    for (let n = startNum; n <= tipNum; n += step) {
        ...
        const entries = await api.query.cyruxMining.miners.entriesAt(h);
