...
function humanToTyped(api, data) {
    ...
    return api.createType('cyruxTypesMessagingTokenomicParameters', convertedParams);
}

function createUpdateCall(api, data) {
    return api.tx.cyruxComputation.updateTokenomic(data)
}

async function readFromChain(api) {
    const params = await api.query.cyruxComputation.tokenomicParameters();
