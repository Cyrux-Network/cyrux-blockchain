...
async function estimateFeeOn(endpoint, callHex) {
    // Create another API because we are not querying cyrux
...
program
    .option('--chain, --substrate-ws-endpoint <url>', 'Substrate WS endpoint. Supported builtin endpoints: cyrux, khala.', process.env.ENDPOINT || 'ws://localhost:9944')
...
chain
    ...
    .action(run(async (contractId, plainCommand, options) => {
        ...
        const call = api.tx.cyrux.pushCommand(
            cid,
            JSON.stringify({
...
chain
    ...
    .action(run (async (workerKey) => {
        ...
        let [workerInfo, miner, pid] = await Promise.all([
            api.query.cyruxRegistry.workers(workerKey),
            api.query.cyruxComputation.workerBindings(workerKey),
            api.query.cyruxStakePool.workerAssignments(workerKey),
        ]);
        workerInfo = workerInfo.unwrapOr();
        miner = miner.unwrapOr();
        pid = pid.unwrapOr();

        const minerInfo = miner ? await api.query.cyruxComputation.miners(miner) : undefined;
        const poolInfo = pid ? await api.query.cyruxStakePool.stakePools(pid) : undefined;
...
chain
    ...
    .action(run(async (pid) => {
        const api = await useApi();
        const pool = await api.query.cyruxStakePool.stakePools(pid);
    ...
    .action(run(async (opt) => {
        ...
        while (true) {
            ...
            function processExtrinsic(extrinsic) {
                ...
                if (method === 'syncOffchainMessage' && section === 'cyruxMq') {
...
contract
    ...
    .action(run(async (wasmPath) => {
        const api = await useApi();
        // TODO: move to cyruxContract later
        const code = fs.readFileSync(wasmPath);
        const codeHex = '0x' + code.toString('hex');
        const call = api.tx.cyruxRegistry.uploadCode(codeHex);
        await printTxOrSend(call);
    }));
...
contract
    ...
    .action(run(async (codeHash, callData, worker) => {
        const api = await useApi();
        const call = api.tx.cyruxRegistry.instantiateContract(
            { 'WasmCode': codeHash },
            callData,
            '0x',
            worker
