...
async function poolStatus(apiAt, pid, account) {
    ...
    let poolInfo = await apiAt.query.cyruxStakePool.stakePools(pid);
    if (poolInfo.isSome) {
    ...
    let poolStaker = await apiAt.query.cyruxStakePool.poolStakers([pid, account]);
...
async function main() {
    ...
    for (let blockNumber = since; blockNumber <= until; blockNumber += step) {
        ...
        const lock = locks.filter(l => l.id.toHuman() == 'cyrux/sp');
        if (lock.length > 0) {
