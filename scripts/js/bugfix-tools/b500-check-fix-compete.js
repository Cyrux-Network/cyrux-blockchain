...
async function main() {
    ...
    let pools = await apiAt.query.cyruxStakePool.stakePools.entries();
    pools = pools
    ...
    // Dump stakers & miners
    let stakes = await apiAt.query.cyruxMining.stakes.entries();
    stakes = stakes.map(([key, value]) => [key.args[0].toString(), value.unwrap()]);
    let miners = await apiAt.query.cyruxMining.miners.entries();
