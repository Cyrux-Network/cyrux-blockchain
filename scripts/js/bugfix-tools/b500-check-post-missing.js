...
async function printMiner(api, pid, worker, checks) {
    ...
    async function print(n) {
        ...
        const m = await apiAt.query.cyruxMining.miners(miner);
        const worker = await apiAt.query.cyruxMining.minerBindings(miner);
        const stake = await apiAt.query.cyruxMining.stakes(miner);
        const pool = await apiAt.query.cyruxStakePool.stakePools(pid);
        console.log({
            n,
            miner: m.toHuman(),
