...
use cyrux_types::contract::ContractId;
...
#[derive(Debug, Parser)]
#[clap(name = "cyrux Debug Utility CLI")]
enum Cli {
...
async fn main() {
    ...
    match cli {
        Cli::DecodeWorkerRegistrationInfo {
            ...
            let msg: cyrux_types::WorkerRegistrationInfo<sp_runtime::AccountId32> =
        ...
        Cli::DecodeSignedMessage { hex_data } => {
            use cyrux_types::messaging::SignedMessage;
...
async fn handle_pink_command(command: PinkCommand) {
    ...
    match command {
        ...
        PinkCommand::Command { id, message } => {
            ...
            println!(
                "topic: (0x{})",
                hex::encode(cyrux_types::contract::command_topic(id))
            );
...
fn decode_mq_payload(destination: &[u8], payload: &[u8]) {
    ...
    try_decode_with_types!(
        ...
        GatekeeperEvent,
        cyrux_pallets::registry::RegistryEvent,
    );
