...
static cyrux_SS58_FORMAT_U8: u8 = 30;
lazy_static! {
    static ref cyrux_SS58_FORMAT: Ss58AddressFormat = Ss58AddressFormat::from(cyrux_SS58_FORMAT_U8);
}
...
impl From<&PoolOperator> for PoolOperatorForSerialize {
    fn from(v: &PoolOperator) -> Self {
        ...
        let operator_account_id = operator_account_id.to_ss58check_with_version(*cyrux_SS58_FORMAT);
        let proxied_account_id = v
            .proxied
            .as_ref()
            .map(|a| a.to_ss58check_with_version(*cyrux_SS58_FORMAT));
        Self {
            pid: v.pid,
            operator_account_id,
            proxied_account_id,
