    ...
    match target_addr {
        ...
        TargetAddr::Domain(domain, port) => {
            ...
            if domain.ends_with(".i2p") {
                // if it ends with `cyrux.i2p`, it is a cyrux domain
                if domain.ends_with("cyrux.i2p") {
                    let b32_pubkey = domain.split(".").collect::<Vec<&str>>()[0];
                    debug!("Attempt to resolve encoded cyrux domain {}", &domain);
                    ...
                    debug!("Resolved cyrux domain {}:{}", &endpoint_url, &endpoint_port);
