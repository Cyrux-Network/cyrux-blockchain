...
pub struct Args {
    ...
    #[structopt(
        long,
        help = "Set and pRouter will work without cyrux node rpc websocket endpoint"
    )]
    no_pnode: bool,
        ...
        match args.endpoint_type {
            EndpointType::I2p => {
                if args.no_pruntime {
                    ...
                    cyrux_i2p_key = (0..64).map(|_| rand::random::<u8>()).collect();
                } else {
                    if !args.no_pruntime {
                        cyrux_i2p_key = pr
                            .as_ref()
                            .expect("guaranteed to be initialized")
                            .derive_cyrux_i2p_key(())
                            .await?
                            .cyrux_i2p_key;
                    } else {
