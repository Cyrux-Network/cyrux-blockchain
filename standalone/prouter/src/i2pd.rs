...
// Here `cyrux_i2p_key` is the private key derived from the pRuntime private key with `cyruxI2PKey` info.
// check `derive_cyrux_i2p_key()` method in `prpc_service.rs`
pub fn generate_ident_to_file(
    abs_datadir: &PathBuf,
    filename: String,
    cyrux_i2p_key: Vec<u8>,
) -> Result<String> {
    let mut keyfile_path = abs_datadir.clone();
    keyfile_path.push(filename);

    let c_sk: *const c_char = cyrux_i2p_key.as_ptr() as *const c_char;
