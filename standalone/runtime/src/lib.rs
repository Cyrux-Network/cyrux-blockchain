...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("cyrux-node"),
    impl_name: create_runtime_str!("cyrux-node"),
    authoring_version: 1,
...
impl cyrux_pallets::cyruxConfig for Runtime {
    ...
    fn filter(&self, c: &RuntimeCall) -> bool {
        match self {
            ...
            ProxyType::StakePoolManager => matches!(
                c,
                RuntimeCall::Utility { .. }
                    | RuntimeCall::cyruxStakePoolv2(pallet_stake_pool_v2::Call::add_worker { .. })
                    | RuntimeCall::cyruxStakePoolv2(
                    ...
                    | RuntimeCall::cyruxStakePoolv2(pallet_stake_pool_v2::Call::create { .. })
                    | RuntimeCall::cyruxRegistry(pallet_registry::Call::register_worker { .. })
                    | RuntimeCall::cyruxRegistry(pallet_registry::Call::register_worker_v2 { .. })
                    | RuntimeCall::cyruxMq(pallet_mq::Call::sync_offchain_message { .. })
            ),
        }
    }
...
impl pallet_computation::Config for Runtime {
    ...
    type Randomness = RandomnessCollectiveFlip;
    type OnReward = cyruxStakePoolv2;
    type OnUnbound = cyruxStakePoolv2;
    type OnStopped = cyruxStakePoolv2;
...
impl pallet_rmrk_core::Config for Runtime {
    ...
    type TransferHooks = cyruxWrappedBalances;
...
parameter_types! {
    pub EIP712Name: Vec<u8> = b"cyruxNetwork".to_vec();
...
construct_runtime!(
    pub struct Runtime {
        ...
        // cyrux
        cyruxMq: pallet_mq,
        cyruxRegistry: pallet_registry,
        cyruxComputation: pallet_computation,
        cyruxStakePoolv2: pallet_stake_pool_v2,
        cyruxStakePool: pallet_stake_pool,
        cyruxVault: pallet_vault,
        cyruxWrappedBalances: pallet_wrapped_balances,
        cyruxBasePool: pallet_base_pool,
        cyruxPhatContracts: pallet_phat,
        cyruxPhatTokenomic: pallet_phat_tokenomic,

        // Rollup and Oracles
        PhatRollupAnchor: pallet_anchor = 100,
        PhatOracle: pallet_oracle = 101,

        // Put them here to make sure pherry could be compiled with cyrux's metadata.
...
impl_runtime_apis! {
    ...
    impl pallet_mq_runtime_api::MqApi<Block> for Runtime {
        fn sender_sequence(sender: &cyrux_types::messaging::MessageOrigin) -> Option<u64> {
            cyruxMq::offchain_ingress(sender)
        }
    }
